...
- (instancetype)init
{
    ...
    if (err) {
        NSLog(@"NewMusicSequence() failed with error %d in %s.", (int)err, __PRETTY_FUNCTION__);
        return nil;
    }
...
- (instancetype)initWithData:(NSData *)data error:(NSError **)error
{
    ...
    if (err) {
        NSLog(@"MusicSequenceFileLoadData() failed with error %d in %s.", (int)err, __PRETTY_FUNCTION__);
		*error = [NSError errorWithDomain:NSOSStatusErrorDomain code:err userInfo:nil];
        return nil;
    }
...
- (instancetype)initWithMusicSequence:(MusicSequence)musicSequence error:(NSError **)error
{
	...
	if (self = [super init]) {
		...
		if (err) {
			NSLog(@"MusicSequenceSetUserCallback() failed with error %d in %s.", (int)err, __PRETTY_FUNCTION__);
			*error = [NSError errorWithDomain:NSOSStatusErrorDomain code:err userInfo:nil];
			return nil;
		}
		...
		if (err) {
			NSLog(@"MusicSequenceGetTempoTrack() failed with error %d in %s.", (int)err, __PRETTY_FUNCTION__);
			*error = [NSError errorWithDomain:NSOSStatusErrorDomain code:err userInfo:nil];
			return nil;
		}
		...
		if (err) {
			NSLog(@"MusicSequenceGetTrackCount() failed with error %d in %s.", (int)err, __PRETTY_FUNCTION__);
			*error = [NSError errorWithDomain:NSOSStatusErrorDomain code:err userInfo:nil];
			return nil;
		}
		...
		for (UInt32 i = 0; i < numTracks; i++) {
			...
			if (err){
				NSLog(@"MusicSequenceGetIndTrack() failed with error %d in %s.", (int)err, __PRETTY_FUNCTION__);
				*error = [NSError errorWithDomain:NSOSStatusErrorDomain code:err userInfo:nil];
				return nil;
			}
...
- (void)dealloc
{
    ...
    if (err) NSLog(@"DisposeMusicSequence() failed with error %d in %s.", (int)err, __PRETTY_FUNCTION__);
}
...
- (MIKMIDITrack *)addTrack
{
    ...
    if (err) {
        NSLog(@"MusicSequenceNewTrack() failed with error %d in %s.", (int)err, __PRETTY_FUNCTION__);
        return nil;
    }
...
- (BOOL)removeTrack:(MIKMIDITrack *)track
{
    ...
    if (err) {
        NSLog(@"MusicSequenceDisposeTrack() failed with error %d in %s.", (int)err, __PRETTY_FUNCTION__);
        return NO;
    }
...
static void MIKSequenceCallback(void *inClientData, MusicSequence inSequence, MusicTrack inTrack, MusicTimeStamp inEventTime, const MusicEventUserData *inEventData, MusicTimeStamp inStartSliceBeat, MusicTimeStamp inEndSliceBeat)
{
    ...
    if (err) {
        NSLog(@"MusicSequenceGetTrackIndex() failed with error %d in %s.", (int)err, __PRETTY_FUNCTION__);
        return;
    }
...
- (Float64)durationInSeconds
{
    ...
    if (err) NSLog(@"MusicSequenceGetSecondsForBeats() failed with error %d in %s.", (int)err, __PRETTY_FUNCTION__);
    return duration;
}
...
- (NSData *)dataValue
{
    ...
    if (err) {
        NSLog(@"MusicSequenceFileCreateData() failed with error %d in %s.", (int)err, __PRETTY_FUNCTION__);
        return nil;
    }
