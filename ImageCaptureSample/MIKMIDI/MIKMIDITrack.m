...
- (instancetype)initWithSequence:(MIKMIDISequence *)sequence musicTrack:(MusicTrack)musicTrack
{
    if (self = [super init]) {
        ...
        if (err) NSLog(@"MusicTrackGetSequence() failed with error %d in %s.", (int)err, __PRETTY_FUNCTION__);
...
- (BOOL)insertMIDIEvent:(MIKMIDIEvent *)event
{
    ...
    switch (event.eventType) {
        ...
        case kMusicEventType_ExtendedNote:
            ...
            if (err) NSLog(@"MusicTrackNewExtendedNoteEvent() failed with error %d in %s.", (int)err, __PRETTY_FUNCTION__);
            break;

        case kMusicEventType_ExtendedTempo:
            err = MusicTrackNewExtendedTempoEvent(track, timeStamp, ((ExtendedTempoEvent *)data)->bpm);
            if (err) NSLog(@"MusicTrackNewExtendedTempoEvent() failed with error %d in %s.", (int)err, __PRETTY_FUNCTION__);
            break;

        case kMusicEventType_User:
            err = MusicTrackNewUserEvent(track, timeStamp, data);
            if (err) NSLog(@"MusicTrackNewUserEvent() failed with error %d in %s.", (int)err, __PRETTY_FUNCTION__);
            break;

        case kMusicEventType_Meta:
            err = MusicTrackNewMetaEvent(track, timeStamp, data);
            if (err) NSLog(@"MusicTrackNewMetaEvent() failed with error %d in %s.", (int)err, __PRETTY_FUNCTION__);
            break;

        case kMusicEventType_MIDINoteMessage:
            err = MusicTrackNewMIDINoteEvent(track, timeStamp, data);
            if (err) NSLog(@"MusicTrackNewMIDINoteEvent() failed with error %d in %s.", (int)err, __PRETTY_FUNCTION__);
            break;

        case kMusicEventType_MIDIChannelMessage:
            err = MusicTrackNewMIDIChannelEvent(track, timeStamp, data);
            if (err) NSLog(@"MusicTrackNewMIDIChannelEvent() failed with error %d in %s.", (int)err, __PRETTY_FUNCTION__);
            break;

        case kMusicEventType_MIDIRawData:
            err = MusicTrackNewMIDIRawDataEvent(track, timeStamp, data);
            if (err) NSLog(@"MusicTrackNewMIDIRawDataEvent() failed with error %d in %s.", (int)err, __PRETTY_FUNCTION__);
            break;

        case kMusicEventType_Parameter:
            err = MusicTrackNewParameterEvent(track, timeStamp, data);
            if (err) NSLog(@"MusicTrackNewParameterEvent() failed with error %d in %s.", (int)err, __PRETTY_FUNCTION__);
            break;

        case kMusicEventType_AUPreset:
            err = MusicTrackNewAUPresetEvent(track, timeStamp, data);
            if (err) NSLog(@"MusicTrackNewAUPresetEvent() failed with error %d in %s.", (int)err, __PRETTY_FUNCTION__);
            break;
    }
...
- (BOOL)moveEventsFromStartingTimeStamp:(MusicTimeStamp)startTimeStamp toEndingTimeStamp:(MusicTimeStamp)endTimeStamp byAmount:(MusicTimeStamp)offsetTimeStamp
{
    ...
    if (err) NSLog(@"MusicTrackMoveEvents() failed with error %d in %s.", (int)err, __PRETTY_FUNCTION__);
    return !err;
}
...
- (BOOL)clearEventsFromStartingTimeStamp:(MusicTimeStamp)startTimeStamp toEndingTimeStamp:(MusicTimeStamp)endTimeStamp
{
    ...
    if (err) NSLog(@"MusicTrackClear() failed with error %d in %s.", (int)err, __PRETTY_FUNCTION__);
    return !err;
}
...
- (BOOL)cutEventsFromStartingTimeStamp:(MusicTimeStamp)startTimeStamp toEndingTimeStamp:(MusicTimeStamp)endTimeStamp
{
    ...
    if (err) NSLog(@"MusicTrackCut() failed with error %d in %s.", (int)err, __PRETTY_FUNCTION__);
    return !err;
}
...
- (BOOL)copyEventsFromMIDITrack:(MIKMIDITrack *)origTrack fromTimeStamp:(MusicTimeStamp)startTimeStamp toTimeStamp:(MusicTimeStamp)endTimeStamp andInsertAtTimeStamp:(MusicTimeStamp)destTimeStamp
{
    ...
    if (err) NSLog(@"MusicTrackCopyInsert() failed with error %d in %s.", (int)err, __PRETTY_FUNCTION__);
    return !err;
}
...
- (BOOL)mergeEventsFromMIDITrack:(MIKMIDITrack *)origTrack fromTimeStamp:(MusicTimeStamp)startTimeStamp toTimeStamp:(MusicTimeStamp)endTimeStamp atTimeStamp:(MusicTimeStamp)destTimeStamp
{
    ...
    if (err) NSLog(@"MusicTrackMerge() failed with error %d in %s.", (int)err, __PRETTY_FUNCTION__);
    return !err;
}
...
- (BOOL)getTrackNumber:(UInt32 *)trackNumber
{
    ...
    if (err) NSLog(@"MusicSequenceGetTrackIndex() failed with error %d in %s.", (int)err, __PRETTY_FUNCTION__);
    return !err;
}
...
- (MusicTrackLoopInfo)loopInfo
{
    ...
    if (err) NSLog(@"MusicTrackGetProperty() failed with error %d in %s.", (int)err, __PRETTY_FUNCTION__);
    return info;
}
...
- (void)setLoopInfo:(MusicTrackLoopInfo)loopInfo
{
    ...
    if (err) NSLog(@"MusicTrackSetProperty() failed with error %d in %s.", (int)err, __PRETTY_FUNCTION__);
}
