...
- (instancetype)init
{
    if (self = [super init]) {
        ...
        if (err) {
            NSLog(@"NewMusicPlayer() failed with error %d in %s.", (int)err, __PRETTY_FUNCTION__);
            return nil;
        }
...
- (void)dealloc
{
    ...
    if (err) NSLog(@"DisposeMusicPlayer() failed with error %d in %s.", (int)err, __PRETTY_FUNCTION__);
}
...
- (void)preparePlayback
{
    ...
    if (err) NSLog(@"MusicPlayerPreroll() failed with error %d in %s.", (int)err, __PRETTY_FUNCTION__);
}
...
- (void)startPlaybackFromPosition:(MusicTimeStamp)position
{
    ...
    if (err) return NSLog(@"MusicPlayerSetTime() failed with error %d in %s.", (int)err, __PRETTY_FUNCTION__);
    ...
    if (err) return NSLog(@"MusicSequenceGetSecondsForBeats() failed with error %d in %s.", (int)err, __PRETTY_FUNCTION__);

    err = MusicPlayerStart(self.musicPlayer);
    if (err) return NSLog(@"MusicPlayerStart() failed with error %d in %s.", (int)err, __PRETTY_FUNCTION__);
...
- (void)stopPlayback
{
    ...
    if (err) {
        NSLog(@"MusicPlayerIsPlaying() failed with error %d in %s.", (int)err, __PRETTY_FUNCTION__);
    }
    ...
    if (musicPlayerIsPlaying) {
        ...
        if (err) {
            NSLog(@"MusicPlayerStop() failed with error %d in %s.", (int)err, __PRETTY_FUNCTION__);
        }
    }
...
- (void)addClickTrackWhenNeededFromTimeStamp:(MusicTimeStamp)fromTimeStamp
{
	...
	if (err) {
		NSLog(@"MusicTrackGetProperty() failed with error %d in %s.", (int)err, __PRETTY_FUNCTION__);
		return;
	}
...
- (void)setSequence:(MIKMIDISequence *)sequence
{
    if (sequence != _sequence) {
        ...
        if (err) return NSLog(@"MusicPlayerSetSequence() failed with error %d in %s.", (int)err, __PRETTY_FUNCTION__);
...
- (MusicTimeStamp)currentTimeStamp
{
    ...
    if (err) NSLog(@"MusicPlayerGetTime() failed with error %d in %s.", (int)err, __PRETTY_FUNCTION__);
    return position;
}
...
- (void)setCurrentTimeStamp:(MusicTimeStamp)currentTimeStamp
{
    ...
    if (err) NSLog(@"MusicPlayerSetTime() failed with error %d in %s.", (int)err, __PRETTY_FUNCTION__);
}
